...
// source is the user's own Contacts database — local, instant, free, and
// never leaves this Mac. If a person is not in Contacts the view falls back
// to the coloured initial — never a generic person glyph.
//
// A Google Workspace directory lookup lived here through 2026-08-09: it sent
// an attendee's address to the People API under the user's own OAuth token
// and put a real photo on colleagues Contacts had never heard of. It was
// removed with the directory.readonly OAuth scope — that scope is Google
// "restricted" tier, which gates OAuth verification behind a paid third-party
// CASA security assessment. Not a cost worth carrying for a feature that only
// decorates avatars; see GoogleOAuth.swift's scope comment for the full
// reasoning. Contacts-only is the fallback that remains.
